...
def read_bfbs(buf, log=None):
    ...
    with open(filepath, 'rb') as f:
        ...
        for i in range(num_enums):
            ...
            #log.info('Processing enum {} ("{}")'.format(i, enum_name))
        ...
        for i in range(_schema.ObjectsLength()):
            ...
            #log.info('Processing object {} ("{}")'.format(i, obj_name))
            ...
            for j in range(num_fields):
                ...
                #log.info('Processing field {} ("{}")'.format(i, field_name))
                ...
                if _field_element == 'object':
                    #el = _field_type.Element()
                    el = _schema.Objects(_field_type.Element())
                    #log.info('YYYYYYYYY {} {}', el, el.IsStruct(), el.Name())
                    if type(el) == object and hasattr(el, 'IsStruct'):
                        _field_element = 'struct' if el.Element().IsStruct() else 'table'
                    else:
                        #log.info('XXXXXXXXXXX', obj_name, _field_element, _field_type, el)
                        _field_element = 'object'
